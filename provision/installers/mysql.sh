...
sudo bash -c 'mysql --user="root" -e "ALTER USER 'root'@'localhost' IDENTIFIED BY '$db_root_password';"'
sudo bash -c ' mysql --user="root" -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' WITH GRANT OPTION;"'
sudo bash -c ' mysql --user="root" -e "CREATE USER 'homestead'@'0.0.0.0' IDENTIFIED BY 'secret';"'
sudo bash -c ' mysql --user="root" -e "CREATE USER 'homestead'@'%' IDENTIFIED BY 'secret';"'
sudo bash -c ' mysql --user="root" -e "GRANT ALL PRIVILEGES ON *.* TO 'homestead'@'0.0.0.0' WITH GRANT OPTION;"'
sudo bash -c ' mysql --user="root" -e "GRANT ALL PRIVILEGES ON *.* TO 'homestead'@'%' WITH GRANT OPTION;"''
sudo bash -c ' mysql --user="root" -e "FLUSH PRIVILEGES;"''
